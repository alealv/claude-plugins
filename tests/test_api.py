...
def test_exposed_objects(modulelevel_internal_objects: list[griffe.Object | griffe.Alias]) -> None:
    """All public objects in the internal API are exposed under `claude_tools`."""
    not_exposed = [
        obj.path
        for obj in modulelevel_internal_objects
        if obj.name not in claude_tools.__all__ or not hasattr(claude_tools, obj.name)
    ]
...
def test_single_locations(public_api: griffe.Module) -> None:
    ...
    multiple_locations = {}
    for obj_name in claude_tools.__all__:
...
def test_inventory_matches_api(
    ...
    public_api_paths.add("claude_tools")
    for item in inventory.values():
        if (
            item.domain == "py"
            and "(" not in item.name
            and (item.name == "claude_tools" or item.name.startswith("claude_tools."))
        ):
